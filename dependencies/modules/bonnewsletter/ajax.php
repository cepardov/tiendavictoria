...
if (Configuration::get('PS_TOKEN_ENABLE') == 1 &&
    ...
    if (Tools::isEmpty($email) || !Validate::isEmail($email)) {
        die(json_encode(array('success' => 3, 'error' => $module->l('Invalid email address.'))));
    } else {
        if ($check > 0) {
            die(json_encode(array('success' => 1, 'error' => $module->l('This email address is already registered.'))));
        } else {
            if (!ClassNewsletter::isRegistered($check)) {
                ...
                die(json_encode(array('success' => 0, 'error' => $module->l('You have successfully subscribed to this newsletter.'))));
            }
        }
    }
}
