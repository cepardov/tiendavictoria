...
{block name='content'}
    {if isset($collection) && $collection}
        <section id="boncollection-page" class="revealOnScroll animated fadeInUp" data-animation="fadeInUp">
            ...
            <div class="row boncollection-wrapper">
                ...
                <div class="boncollection-info col-12 col-xs-12 col-md-6">
                    ...
                    {if isset($collection.title) && $collection.title}
                        <h2 class="col-lg-6">{$collection.title|escape:'htmlall':'UTF-8'}</h2>
                    {/if}
                    {if isset($collection.description) && $collection.description}
                        <p class="boncollection-description">{$collection.description|escape:'htmlall':'UTF-8' nofilter}</p>
                    {/if}
                    <hr>
                    {if isset($collection.author_name) && $collection.author_name}
                        <p class="author">{l s='by' mod='boncollection'} {$collection.author_name|escape:'htmlall':'UTF-8'} </p>
                    {/if}
                                    ...
                                    <a data-caption="{$item.title|escape:'htmlall':'UTF-8'}" data-fancybox="gallery"
                                        ...
                                        <div class="boncollection-item-description">
                                            ...
                                            {if isset($item.description) && $item.description}
                                              <p class="col-xs-12 col-12 col-md-10">{$item.description|escape:'htmlall':'UTF-8' nofilter}</p>  
                                            {/if}
                                        </div>
                                    </a>
                                {/if}
                    ...
                    <div class="boncollection-social social-sharing">
                        ...
                        <ul>
                            ...
                            <li class="facebook"><a target="_blank"
                                    href="https://www.facebook.com/sharer.php?u={$collection_url|escape:'htmlall':'UTF-8'}"></a>
                            </li>
                            <li class="twitter"><a target="_blank"
                                    href="https://twitter.com/intent/tweet?text={$collection.title|escape:'htmlall':'UTF-8'}{$collection_url|escape:'htmlall':'UTF-8'}"></a>
                            </li>
                            <li class="pinterest">
                                <a target="_blank"
                                    href="https://www.pinterest.com/pin/create/button/?media={$urls.base_url|escape:'htmlall':'UTF-8'}modules/boncollection/views/img/{$collection.image|escape:'htmlall':'UTF-8'}&url={$collection_url|escape:'htmlall':'UTF-8'}"></a>
                            </li>
                        </ul>
                    </div>
                {/if}
