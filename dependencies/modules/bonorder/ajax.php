...
if (Configuration::get('PS_TOKEN_ENABLE') == 1 &&
    ...
    if (Tools::isEmpty($bon_order_name) || !Validate::isGenericName($bon_order_name)) {
        die(json_encode(array('success' => 3, 'error' => 'Invalid your name.')));
    } elseif (Tools::isEmpty($bon_order_phone) || !Validate::isPhoneNumber($bon_order_phone)) {
        die(json_encode(array('success' => 2, 'error' => 'Invalid phone number.')));
    } elseif (Tools::isEmpty($bon_order_mail) || !Validate::isEmail($bon_order_mail)) {
            die(json_encode(array('success' => 4, 'error' => 'Invalid email.')));
    } else {
        ...
        if (!Mail::Send(
            ...
            die(json_encode(array('success' => 1, 'alert' => 'Thank you. We will call you soon.')));
        } else {
            die(json_encode(array('success' => 1, 'alert' => 'Thank you. We will call you soon.')));
        }
    }
