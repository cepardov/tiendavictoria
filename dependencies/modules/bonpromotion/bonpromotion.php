...
class Bonpromotion extends Module
{
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 1; ++$i) {
            ...
            $item->sort_order = $i;
            $item->data_start = $now;
            $item->data_end = '2022-01-29 18:55:51';
            foreach ($languages as $language) {
                $item->title[$language['id_lang']] = 'parallax';
                $item->url[$language['id_lang']] = '6-accessories';
                $item->image[$language['id_lang']] = 'sample-'.$i.'.jpg';
                $item->description[$language['id_lang']] = '<h4>Special Offer</h4><h3>new season <strong>sale off 50%</strong></h3>';
            }
    ...
    public function hookHeader()
    {
        ...
        Media::addJsDefL('countdown_days', $this->l('days'));
        Media::addJsDefL('countdown_hr', $this->l('hr'));
        Media::addJsDefL('countdown_min', $this->l('min'));
        Media::addJsDefL('countdown_sec', $this->l('sec'));
